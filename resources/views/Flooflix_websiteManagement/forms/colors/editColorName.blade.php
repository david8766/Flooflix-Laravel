        ...
        <div class="row pt-5 pb-5">
            ...
            <div class="col-lg-6 bg-white">
                <header class="text-center mt-4">
                    <h1 class="font-alfa black">Modifier le nom de la couleur :</h1>
                </header>
                <form action="{{ action('ColorController@updateColorName',$color) }}" class="mt-5 mx-5" method="POST">
                    @csrf
                    <div class="form-group mt-5">
                        <label for="name" class="font-alfa black">Titre</label>
                    <input type="text" name="name" class="form-control {{ $errors->has('name') ? ' is-invalid' : '' }}" value="{{ old('name') }}" placeholder="{{ $color->name}}">
                        @if ($errors->has('name'))
